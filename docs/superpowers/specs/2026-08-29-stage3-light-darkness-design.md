...
- **显示上限处理（用户拍板 2026-08-29）**：**手牌显示上限 10 → 15 张**（`renderHand` 的 `slice(0, 10)` 与 +N 徽标阈值改 15），同时**加长手牌区与挡板**（.hand 区域、.hand-shield 按新宽度自适应），**刷新按钮位置随之调整**；`#app` 加宽（max-width 1780 → ~2000px、左右 padding 140 → 100px，内容宽足够容纳 15 张扇形卡 ≈ 1530px + 中间指示）
- 幽灵牌仍追加在手牌末尾；15 张上限下真卡+幽灵合计最大 ≈ 15 真卡 + 若干幽灵——**幽灵不再触发揭示条方案**（原 2.5 的 `.reveal-strip` 方案废弃）
- 幽灵牌始终：不参与任何事件/手牌计数；抽牌/回手定位查询已排除 `.reveal-ghost`

### 2.6 牌库区（deck area，用户拍板）

每位玩家一个**牌库区**，用于实时显示牌库剩余数量，并与抽牌特效联动：

- **样式**：多张背面朝上的卡牌**叠在一起**（层叠背面卡）；按剩余数量设**多个厚度状态**（层数随数量变化）：
  - `0` 张：空槽占位（无卡背层，中央显示 0 或「空」）
  - `1-3`：1 层 / `4-7`：2 层 / `8-12`：3 层 / `13-18`：4 层（层数 = clamp(ceil(数量/4), 1, 4)，层间微偏移模拟厚度）
- **数量显示**：最顶层背面卡**中央**显示牌库剩余张数（数字徽标）
- **位置**：牌库区位于**刷新按钮的左/右边**（P1 左侧、P2 右侧），与刷新按钮保持间距（≥ 16px，刷新按钮不被遮挡/看不清）；手牌区按 15 张加长后一并重排
- **与抽牌特效联动（关键）**：抽牌幽灵卡的**起始位置 = 牌库区位置**（刷新抽牌 + 牌堆顶打出两种特效都从牌库区出发）：
  - 刷新手牌：`playDrawAnimation` 的 `startX` 改为牌库区外侧（原为手牌区外侧）
  - 牌堆顶打出（`card:deck-played`）：幽灵卡从牌库区出发飞入目标链路堆叠末尾
  - 实现：`deckPos(player)` 统一取牌库区 rect；幽灵卡尺寸沿用 130×178.8

### 2.7 手牌「翻面」按钮 restyle（用户拍板）

现有 `.play-btn` 样式过丑——重做为与游戏一致的石青色霓虹风格（参考 `.choice-bar`/`.diag-btn` 的青色描边 + 发光），悬停亮起；不改交互逻辑（选中卡上缘居中、单击切换正反面）。
