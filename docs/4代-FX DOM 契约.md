...
| 20 | `.rot-180` | `fx-orient.ts` | **场上卡 180° 倒置态（远程页对手一侧；`render-net.ts` 按座位挂）**。产出点两处：`render.ts:254`（`orient === 180 → node.classList.add('rot-180')`，由 `render-net.ts:393` 的 `orient: isSelfSeat ? 0 : 180` 驱动）与 `render.ts:130`（`.protocol-img.rot-180`，由 `render-net.ts:396` 的 `renderProtocolCell(s, player, line, isSelfSeat ? 0 : 180)` 驱动）。**与 ±90° 的区别（这是本条的登记理由）**：180° **不交换布局盒宽高**、只绕中心转 180°（`cloneBoxSwaps(180) === false`，见 `src/ui/fx-orient.ts:51` 与 `tests/ui/fx-orient.test.ts` 的"足迹公式"一节）—— 所以它**不能**复用 ±90° 的浮层卡建盒路径：拿 ±90° 冒充 180° 会得到"朝向对但尺寸错"的假正确，反之亦然。小注：热座页也有 `.rot-180` 产出点（`render.ts:130`），但那个节点是 `.protocol-img`，**不是 FX 节点**（FX 侧的协议图走 `img` 钩子 + 克隆），故本钩子在热座页对 FX 实际不可达 —— 它是**远程页专属**的契约项 |
