...
/**
 * 3代 柔性 flexibility（关键词：回手/平移/翻转/抽牌/交换；座右铭：随机应变）。
 * 权威卡文：src/data/cards3.ts（compile3文本.txt）；裁决：docs/3代-批3-规格与裁决清单.md
 * （RQ7-A 回手任意场牌回其主；「回手或平移」= select-action 二选一）。
 */
...
/** flexibility-0 中：回手或平移1张牌。 */
function* flexibility0Middle(ctx: EffectCtx): Generator<EffectStep, void, StepResult> {
  const act = yield {
    kind: 'select-action', title: 'flexibility-0：回手或平移1张牌', min: 1, max: 1, optional: false, candidates: [],
    actions: ['action:return', 'action:shift'],
  };
  if (act.selected.length === 0) return;
  const cand = ctx.candidates({ zone: 'field' });
  const tAns = yield { kind: 'select', title: act.selected[0] === 'action:return' ? 'flexibility-0：回手这张牌' : 'flexibility-0：平移这张牌', min: 1, max: 1, optional: false, candidates: cand };
  ...
  const lAns = yield {
    kind: 'select-line', title: 'flexibility-0：平移到哪条链路', min: 1, max: 1, optional: false, candidates: [],
...
/** flexibility-1 中：翻转或平移你的1张牌。 */
function* flexibility1Middle(ctx: EffectCtx): Generator<EffectStep, void, StepResult> {
  const act = yield {
    kind: 'select-action', title: 'flexibility-1：翻转或平移你的1张牌', min: 1, max: 1, optional: false, candidates: [],
  ...
  if (srcLine === null) return;
  const lAns = yield {
    kind: 'select-line', title: 'flexibility-1：平移到哪条链路', min: 1, max: 1, optional: false, candidates: [],
    lines: ([0, 1, 2] as Line[]).filter((l) => l !== srcLine),
  };
  if (lAns.selected.length > 0) yield { op: 'shift', uid: tAns.selected[0], targetLine: Number(lAns.selected[0].replace('line:', '')) as Line };
}

/** flexibility-2 顶（end，top 命令被盖仍生效）：结束：若此牌被1张反面朝下的牌覆盖，你可以平移那张牌。
 *  上方相邻覆盖者为 faceDown → 可选把那张 faceDown 平移到别线（覆盖者当前线 → 目标线自选）。 */
function* flexibility2End(ctx: EffectCtx): Generator<EffectStep, void, StepResult> {
  ...
  const act = yield {
    kind: 'select-action', title: 'flexibility-2（结束）：此牌被反面牌覆盖——你可以平移那张牌', min: 1, max: 1, optional: true,
    candidates: [], actions: ['action:shift'],
  };
  if (act.selected.length === 0) return;
  const lAns = yield {
    kind: 'select-line', title: 'flexibility-2：把覆盖者平移到哪条链路', min: 1, max: 1, optional: false, candidates: [],
...
/** flexibility-3 中：平移对手的1张牌，或交换你的2个协议。 */
function* flexibility3Middle(ctx: EffectCtx): Generator<EffectStep, void, StepResult> {
  const act = yield {
    kind: 'select-action', title: 'flexibility-3：平移对手的1张牌，或交换你的2个协议', min: 1, max: 1, optional: false, candidates: [],
  ...
  const tAns = yield { kind: 'select', title: 'flexibility-3：平移对手的1张牌', min: 1, max: 1, optional: false, candidates: cand };
  if (tAns.selected.length === 0) return;
  const card = findCard(ctx.s, tAns.selected[0]);
  const srcLine = card?.line ?? ctx.card.line;
  if (srcLine === null) return;
  const lAns = yield {
    kind: 'select-line', title: 'flexibility-3：平移到哪条链路', min: 1, max: 1, optional: false, candidates: [],
    lines: ([0, 1, 2] as Line[]).filter((l) => l !== srcLine),
  };
  if (lAns.selected.length > 0) yield { op: 'shift', uid: tAns.selected[0], targetLine: Number(lAns.selected[0].replace('line:', '')) as Line };
}
