...
export interface NetTransport {
   ...
   * ## 契约
   *
   *  - 回调在**每条**数据通道 `open` 时**各叫一次**（不是"第一条 open 就叫一次就完"）——
   *    ⚠️ 这**必须**：两条通道的 `open` 不同时到，而一条消息只走其中**一条**（`act` 或 `beat`）
   *    ⇒ 只在第一条 open 时叫一次的话，走另一条的那条消息就再也没有机会（真机实测见
   *    `.superpowers/g5-T8/ice-diag-read.txt`：`hello` 走 `beat`，`act` 先 open ⇒ 那次重试仍
   *    报 `offline`，之后彻底停住）；
   *  - 通道**已经** open 时才订阅 ⇒ 实现应当对那条通道**立刻**叫一次（别让调用方漏掉这个时机）；
   *  - 返回退订函数；
