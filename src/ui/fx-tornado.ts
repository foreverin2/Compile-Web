 ...
 *
 * ⚠️ **G2 修正 R15-A**：这两个数是**热座场上卡**（130.57 × 175）的 90.4% / 97.1% ——
 * 也就是说这个特效的尺寸本来就是**相对卡**定义的（118 / 130.57 = 0.904、
 * 170 / 175 = 0.971），不是一个独立的绝对尺寸。远程页的卡小得多
 * （**实测 93.42 × 127.58**，`--card-h: 130`）⇒ 再用 118 × 170 就变成卡宽的 **126%** /
 * 卡高的 **133%**（**溢出卡外、压到相邻列**）。所以 `buildTornadoFx` 收一个 `scale`：
 * 按"真卡高 ÷ 热座场上卡高（`TORNADO_BASE_CARD_H`）"整体缩放，
 * 盒尺寸 / SVG viewBox / 半径一起缩 ⇒ 与卡的**比例**在两种页面里一致。
 * `scale = 1` 时逐字等于改动前（热座构造性不变）。 */
 ...
 *
 *  ⚠️ **R24 实测更正**：这里原来写着"**远程页**那一档按 140 除以 175 得 0.8" —— 那个算式里的
 *  分子是 `--card-h: 140` 时代的值；**R20 起远程页 `--card-h: 130`**，于是远程页真实的
 *  场上卡高是 **127.578**（`offsetHeight` 128）⇒ 实际系数 ≈ **0.729**（不是那个 0.8）。
 *  本常量**不用改**（它是"热座基准"这个语义，175 仍然对），但读注释的人不该再以为
 *  远程页是那一档 —— 系数由 `stackCardBox().h / 本常量` 在运行期算出来。
 *  （守卫：`tests/ui/net-r24-drag-orient-and-foe-snap.test.ts` 会报红，若有人把那个过期算式写回来。） */
 ...
 * 调用方按"真卡的场上高 ÷ `TORNADO_BASE_CARD_H`"传（远程页**实测** ≈ 127.578 / 175 ≈ 0.729）。
