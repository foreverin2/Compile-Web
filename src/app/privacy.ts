...
/* ─────────────── 授权弹窗（Task 4）的三句话：**同一个唯一出处** ─────────────── */

/**
 * 授权弹窗（`src/ui/local-consent.ts`）上**面向玩家**的三句话。
 *
 * ## 为什么必须住在这里（修复轮 3 · 阻断项）
 *
 * 修复轮 2 把"承诺句哈希钉死"的枚举面钉在 `PRIVACY_COPY` × `PRIVACY_GROUPS` 上，而
 * `CONSENT_COPY.body[0]` / `body[2]` / `denyHint` 当时是 `local-consent.ts` 里的**手写串**
 * ⇒ 它们**根本不在枚举面里**。复验者实测：把 `denyHint` 改写成
 * 「…拒绝时全程零磁盘写入，硬盘上不会留下任何痕迹。」（授权弹窗上玩家可见的一句**全新绝对承诺**）
 * 后 20 条腿**全绿** —— 穿透面与哈希强度**无关**，纯粹是**枚举面**不够：
 * ① 钉住表只遍历 `copyEntries(PRIVACY_COPY)`；② "唯一出处"腿只查"本文件的句子有没有被抄第二份"，
 * 不查"别处有没有新写一句假承诺"；③ `ABSOLUTE_FORMS` 只扫本文件。
 *
 * ## 处置（协调者选定的 (a)：让"唯一出处"从**口号**变成**事实**）
 *
 * 授权弹窗的玩家可见措辞改为**只从本文件渲染**：这三句搬到导出面上，
 * `local-consent.ts` 只**引用**（`tests/ui/local-consent.test.ts` 有一条结构性腿钉住
 * "消费方文件里不许出现任何手写的玩家可见措辞"）。于是承诺句只有一个家，
 * 钉住表的枚举面（`tests/app/privacy.test.ts` 的 `MODULE_PINS`，遍历本模块的**运行期导出面**）
 * 自然覆盖到它们 —— 修的是"承诺句有几个家"，不是哈希强度（换取密码学哈希零收益）。
 *
 * ## 为什么不塞进 `PRIVACY_COPY` / `PRIVACY_GROUPS`
 *
 * `privacyLines()` 的输出同时是 Task 7「本地数据与隐私」**整屏**的渲染源（它的测试断言
 * "屏上含 `privacyLines()` 的每一条"）。这三句是**授权弹窗**语境的措辞（"允许后…"只在弹窗里成立），
 * 塞进分组会改变 `privacyLines()` 的输出集合、与 Task 7 的在飞实现相互干扰，
 * 而**对本任务零增益**：导出面钉死不依赖 `PRIVACY_GROUPS`（后者只按 `PRIVACY_GROUPS` 遍历，
 * 恰恰是这次穿透的成因）。
 */
export const CONSENT_ALLOW_NOTE = '允许后，昵称、游戏设置与卡组会保存在你自己的浏览器里（可以随时清除）。';

/**
 * 拒绝（游客模式）的**后果**：功能全可用、数据只在内存、刷新或关闭即全丢
 * （用户裁决 #6：中性陈述 + 明确后果）。
 */
export const CONSENT_DENY_NOTE = '不允许也能正常游玩全部内容：本次游戏的所有数据只存在内存里，刷新或关闭页面就全部丢掉。';

/** 拒绝后的出路：授权不是一次性的，随时可在主界面改主意（§3.6）。 */
export const CONSENT_DENY_HINT = '你随时可以在主界面的「本地数据与隐私」里改变这个选择。';
